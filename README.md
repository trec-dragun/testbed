...
For OpenRouter runs, the wrapper points Claude Code at OpenRouter's Anthropic-compatible endpoint and passes `OPENROUTER_API_KEY` through both `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN`, because current `--bare` Claude Code builds require `ANTHROPIC_API_KEY` for scripted runs. It also sets Claude Code's model variables (`ANTHROPIC_MODEL`, `ANTHROPIC_DEFAULT_*_MODEL`, and `CLAUDE_CODE_SUBAGENT_MODEL`) to the requested model, and sets `CLAUDE_CODE_EFFORT_LEVEL` to the requested effort. Claude Code over OpenRouter is still provider-sensitive: some non-Anthropic backbones may fail to use tools or may return no final output. Failed topic runs keep the temporary session folder and write `claude_stderr.log` plus `claude_exit_code.txt` under the topic artifact directory; set `CLAUDE_DEBUG_LOG=1` to also save Claude Code debug logs.
